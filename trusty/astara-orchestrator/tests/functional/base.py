...
APPLIANCE_ACTIVE_TIMEOUT = 360
ASTARA_AUTO_ADD_RESOURCES = True
TEST_NET_SUBNET_CIDR = '10.1.1.0/24'
...
def get_config(svc):
    config = yaml.load(
        subprocess.check_output(['juju', 'get', svc]))
    return config['settings']


def get_test_config():
    keystone_config = get_config('keystone')
    keystone_addr = subprocess.check_output(
        ['juju-deployer', '-f', 'keystone']).strip()

    def get_val(k):
        return keystone_config[k]['value']

    return {
        'username': get_val('admin-user'),
        'password': get_val('admin-password'),
        'tenant_name': 'admin',
        'region_name': get_val('region'),
        'service_tenant_name': get_val('service-tenant'),
        'auth_url': 'http://%s:5000/v2.0' % keystone_addr,
    }
...
class ClientManager(object):
    ...
    def keystoneclient(self):
        if not self._keystoneclient:
            client = _keystoneclient.Client(
                session=self.keystone_session,
                auth_url=self.keystone_session.auth.auth_url)
...
class AdminClientManager(ClientManager):
    ...
    def __init__(self):
        ...
        self.service_tenant_id = [
            t.id for t in self.keystoneclient.tenants.list()
            if t.name == self.config['service_tenant_name']][0]
    ...
    def get_router_appliance_server(self, router_uuid, retries=10,
        ...
        if wait_for_active:
            LOG.debug('Waiting for backing instance %s to become ACTIVE',
                      service_instance)
            for i in six.moves.range(APPLIANCE_ACTIVE_TIMEOUT):
...
class TestTenant(object):
    ...
    def setup_networking(self):
        ...
        if not ASTARA_AUTO_ADD_RESOURCES:
            addr = netaddr.IPNetwork(TEST_NET_SUBNET_CIDR)
            subnet_body = {
                'subnet': {
                    'name': data_utils.rand_name(name='subnet', prefix='ak'),
                    'network_id': network['id'],
                    'cidr': TEST_NET_SUBNET_CIDR,
...
class AstaraFunctionalBase(testtools.TestCase):
    ...
    @staticmethod
    def juju_ssh(unit, command):
        return subprocess.check_output([
            'juju', 'ssh', unit] + command.split(' '))
